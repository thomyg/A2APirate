...
public sealed class DictionaryAgent : IAgentHandler
{
    ...
    private static readonly Dictionary<string, string> Knowledge = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["a2a"] = "A2A (Agent-to-Agent) is an open protocol that lets AI agents talk to each other over HTTP using JSON-RPC. Key concepts: AgentCard for discovery, SendMessage for sync, SendStreamingMessage for SSE streaming.",
        ...
        ["json"] = "JSON-RPC 2.0 is the wire format for A2A. Every request has: jsonrpc (always '2.0'), method (like 'SendMessage'), params (the message), and id (for correlation). A2A v1.0 uses PascalCase method names.",
        ["json-rpc"] = "JSON-RPC 2.0 is A2A's wire format. A request looks like: {jsonrpc:'2.0', method:'SendMessage', params:{message:{...}}, id:'1'}. A2A v1.0 uses PascalCase method names like SendMessage, SendStreamingMessage, GetTask.",
        ...
        ["streaming"] = "A2A streaming uses method 'SendStreamingMessage'. The response comes back as SSE events: first a Task (Submitted), then StatusUpdate (Working), then ArtifactUpdate events (one per token), and finally StatusUpdate (Completed).",
